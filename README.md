...
# v1.0 OUT! Check out the releases tab to install!
# eClient
...
def replace_arguments(argstr: str, versionData: Dict[str, Any], path: str, options: MinecraftOptions) -> str:
    name = options.get("token", "{token}")
    result = name or ""
    name2 = options.get("uuid", "{uuid}")
    result2 = name2 or ""
    name3 = versionData.get("assets", versionData["id"])
    result3 = name3 or ""
    name4 = options.get("gameDirectory", path)
    result4 = name4 or ""
    name5 = options.get("username", "{username}")
    result5 = name5 or ""
    name6 = options.get("launcherVersion", get_library_version())
    result6 = name6 or ""
    name7 = options["nativesDirectory"]
    result7 = name7 or ""
    argstr = argstr.replace("${natives_directory}", "{result7}")
    argstr = argstr.replace("${launcher_name}", options.get("launcherName", "minecraft-launcher-lib"))
    argstr = argstr.replace("${launcher_version}", result6)
    argstr = argstr.replace("${classpath}", options["classpath"])
    argstr = argstr.replace("${auth_player_name}", result5)
    argstr = argstr.replace("${version_name}", versionData["id"])
    argstr = argstr.replace("${game_directory}", result4)
    argstr = argstr.replace("${assets_root}", os.path.join(path, "assets"))
    argstr = argstr.replace("${assets_index_name}", result3)
    argstr = argstr.replace("${auth_uuid}", result2)
    argstr = argstr.replace("${auth_access_token}", result)
...
```
## Peace Out!
# Copyright v-pun215 (Vihaan Pundir) 2023
